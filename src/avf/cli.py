...
def build_parser() -> argparse.ArgumentParser:
    ...
    write_report = subparsers.add_parser(
        ...
        help="Run the Phase 4D failure analysis and final analysis report over a Phase 4A metrics table.",
    )
    ...
    write_report.add_argument(
        "--analysis-root",
        help="Directory where Phase 4D report artifacts should be written. Defaults to the metrics table analysis root.",
    )
    write_report.add_argument(
        "--generated-at",
        help="Optional timestamp override for reproducible final report artifacts.",
    )
    write_report.add_argument(
        "--code-version",
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if args.command == "write-analysis-report":
        try:
            ...
            print(f"Phase 4D analysis report failed: {exc}", file=sys.stderr)
            return 1
...
def _phase4d_report_cli_summary(result: Phase4DFailureAnalysisResult) -> Dict[str, object]:
    return {
        "dataset_id": result.dataset_id,
        "experiment_id": result.experiment_id,
        "run_count": result.failure_analysis["run_count"],
        "ordinary_task_outcome_count": result.failure_analysis["ordinary_task_outcome_count"],
        "infrastructure_failure_count": result.failure_analysis["taxonomy_counts"]["infrastructure_failure"],
        "descriptive_only": result.failure_analysis["limitations"]["descriptive_only"],
